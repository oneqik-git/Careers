...
function renderPointList(value, emptyLabel) {
  ...
  if (!items.length) {
    return <p className="oq-detail-content mt-2 text-sm font-normal leading-6 text-[var(--text-soft)]">{emptyLabel}</p>;
  }

  return (
    <ul className="oq-detail-content oq-detail-list mt-2 space-y-1.5 text-sm font-normal leading-6 text-[var(--text-soft)]">
      {items.map((item) => (
        <li key={item}>{item}</li>
      ))}
    </ul>
  ...
  return (
    <PublicShell>
      {isLoading ? (
        ...
        <div className="oq-public-job-detail space-y-6">
          <PageHero
            ...
            actions={[
              ...
              { label: 'Apply', href: `?auth=register&next=${encodeURIComponent(`/candidate/jobs/detail?jobId=${job.id}`)}` },
            ]}
            aside={(
              <div className="space-y-5">
                <div>
                  <p className="text-xs uppercase tracking-[0.28em] text-white/72">Compensation</p>
                  <p className="mt-1.5 text-xl font-semibold">{formatSalaryRange(job.salary_min, job.salary_max, job.salary_disclosed)}</p>
                </div>
          ...
          <div className="oq-public-job-detail-grid grid gap-5">
              ...
              <SectionCard className="oq-public-job-section oq-public-job-section-compact" title="Role overview" description="Enough context to decide whether the role is worth your time.">
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  <div>
                    ...
                    <p className="mt-1.5 text-sm text-[var(--text)]">{job.department || 'Not specified'}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.24em] text-[var(--text-muted)]">Level</p>
                    <p className="mt-1.5 text-sm text-[var(--text)]">{job.level || 'Not specified'}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.24em] text-[var(--text-muted)]">Work mode</p>
                    <p className="mt-1.5 text-sm text-[var(--text)]">{job.work_mode ? formatStatus(job.work_mode) : 'Not specified'}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.24em] text-[var(--text-muted)]">Education</p>
                    <p className="mt-1.5 text-sm text-[var(--text)]">{job.education_requirement || 'Not specified'}</p>
                  </div>
                </div>
                <div className="mt-5">
                  <p className="text-xs uppercase tracking-[0.24em] text-[var(--text-muted)]">Description</p>
                  <p className="oq-detail-content mt-2 whitespace-pre-wrap text-sm font-normal leading-6 text-[var(--text-soft)]">
              ...
              <SectionCard className="oq-public-job-section oq-public-job-section-compact" title="Requirements and fit" description="Hope you got these, Coz we sure need it">
                <div className="grid gap-5 lg:grid-cols-2">
                  <div>
                    ...
                    <div className="oq-detail-content mt-2">{renderSkillGroup(job.required_skills, 'No required skills listed.')}</div>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.24em] text-[var(--text-muted)]">Preferred skills</p>
                    <div className="oq-detail-content mt-2">{renderSkillGroup(job.preferred_skills, 'No preferred skills listed.')}</div>
                  </div>
                </div>
              </SectionCard>
